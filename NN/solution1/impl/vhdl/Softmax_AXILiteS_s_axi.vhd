...
entity Softmax_AXILiteS_s_axi is
generic (
...
end entity Softmax_AXILiteS_s_axi;
...
architecture behave of Softmax_AXILiteS_s_axi is
    ...
    constant ADDR_IN_DATA_CTRL    : INTEGER := 16#14#;
    constant ADDR_OUT_DATA_DATA_0 : INTEGER := 16#18#;
    constant ADDR_OUT_DATA_CTRL   : INTEGER := 16#1c#;
    constant ADDR_SIZE_V_DATA_0   : INTEGER := 16#20#;
    constant ADDR_SIZE_V_CTRL     : INTEGER := 16#24#;
    ...
    signal int_size_V          : UNSIGNED(15 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SIZE_V_DATA_0 =>
                        rdata_data <= RESIZE(int_size_V(15 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    size_V               <= STD_LOGIC_VECTOR(int_size_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SIZE_V_DATA_0) then
                    int_size_V(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_size_V(15 downto 0));
                end if;
            end if;
        end if;
    end process;
