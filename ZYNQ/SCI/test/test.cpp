...
#pragma HLS pipeline
                ...
                // 边界检查，避免访问越界的行数据
                if (i + 2 < IMG_HEIGHT) {
                    line_buffer[c_in][2][j] = input[c_in][i + 2][j]; // 加载新一行
                } else {
                    line_buffer[c_in][2][j] = 0; // 边界外填充0或其他默认值
                }
...
#pragma HLS pipeline
                for (int c_out = 0; c_out < CHANNELS; c_out++) {
                    ...
                    for (int c_in = 0; c_in < CHANNELS; c_in++) {
                        ...
                        for (int ki = 0; ki < KERNEL_SIZE; ki++) {
                            for (int kj = 0; kj < KERNEL_SIZE; kj++) {
                                int ih = i - 1 + ki;  // 卷积核的行索引
                                int jw = j - 1 + kj;  // 卷积核的列索引
                    ...
                    if(mode == 2){
                        ...
                        if (diff != 0) {
                        					output_cache[c_out][j] = x_cache1 / diff; // 计算 A / (A - B)
                                        } else {
                                        	output_cache[c_out][j] = 1; // 当除数为零时，结果设为0（或根据需求设为其他默认值）
                                        }
            ...
            // 更新 line_buffer：上移数据，使得 line_buffer[1] 和 line_buffer[2] 保留前两行
                for (int j = 0; j < IMG_WIDTH; j++) {
#pragma HLS pipeline
                    for (int c_in = 0; c_in < CHANNELS; c_in++) {
		    ...
		    for (int i = 0; i < IMG_HEIGHT; i++) {
		        ...
		        for (int j = 0; j < IMG_WIDTH; j++) {
		                ...
		                for (int c_out = 0; c_out < CHANNELS; c_out++) {
		                    ...
		                    data_t cache = 1.0 / (1.0 + exp(static_cast<double>(-sum))); // 搴旂敤 Sigmoid 鍑芥暟
		                    	data_t x_cache1 = x_cache[c_out][j];
		                        data_t diff = x_cache1 + cache;
		                        if (diff != 0) {
		                        					output_cache[c_out][j] = x_cache1 / diff; // 计算 A / (A - B)
