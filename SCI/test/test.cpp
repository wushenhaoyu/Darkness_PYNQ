...
#pragma HLS UNROLL // 灞曞紑寰幆锛屽姞閫熻绠?
                ...
                // ReLU 鎿嶄綔
                output[c_out][i][j] = (batch_norm_output > 0) ? batch_norm_output : static_cast<data_t>(0); // 濡傛灉 batch_norm_output 澶т簬 0 鍒欒緭鍑猴紝鍚﹀垯杈撳嚭 0
                output[c_out][i][j] = output[c_out][i][j] + window[c_out][1][1];
            }
        }
    }
}
