...
entity LeakyReLU_AXILiteS_s_axi is
generic (
...
end entity LeakyReLU_AXILiteS_s_axi;
...
--        bit 1  - Channel 1 (ap_ready)
--        others - reserved
-- 0x10 : Data signal of channel_V
--        bit 11~0 - channel_V[11:0] (Read/Write)
--        others   - reserved
-- 0x14 : reserved
...
-- 0x34 : reserved
-- 0x38 : Data signal of alpha
--        bit 31~0 - alpha[31:0] (Read/Write)
-- 0x3c : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of LeakyReLU_AXILiteS_s_axi is
    ...
    constant ADDR_CHANNEL_V_DATA_0 : INTEGER := 16#10#;
    constant ADDR_CHANNEL_V_CTRL   : INTEGER := 16#14#;
    ...
    constant ADDR_IN_DATA_DATA_0   : INTEGER := 16#28#;
    constant ADDR_IN_DATA_CTRL     : INTEGER := 16#2c#;
    constant ADDR_OUT_DATA_DATA_0  : INTEGER := 16#30#;
    constant ADDR_OUT_DATA_CTRL    : INTEGER := 16#34#;
    constant ADDR_ALPHA_DATA_0     : INTEGER := 16#38#;
    constant ADDR_ALPHA_CTRL       : INTEGER := 16#3c#;
    ...
    signal int_channel_V       : UNSIGNED(11 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CHANNEL_V_DATA_0 =>
                        rdata_data <= RESIZE(int_channel_V(11 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    channel_V            <= STD_LOGIC_VECTOR(int_channel_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CHANNEL_V_DATA_0) then
                    int_channel_V(11 downto 0) <= (UNSIGNED(WDATA(11 downto 0)) and wmask(11 downto 0)) or ((not wmask(11 downto 0)) and int_channel_V(11 downto 0));
                end if;
            end if;
        end if;
    end process;
