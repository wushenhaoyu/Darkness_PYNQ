...
end entity GELU_AXILiteS_s_axi;
...
architecture behave of GELU_AXILiteS_s_axi is
    ...
    constant ADDR_INPUT_WIDTH_V_DATA_0  : INTEGER := 16#10#;
    constant ADDR_INPUT_WIDTH_V_CTRL    : INTEGER := 16#14#;
    constant ADDR_INPUT_HEIGHT_V_DATA_0 : INTEGER := 16#18#;
    constant ADDR_INPUT_HEIGHT_V_CTRL   : INTEGER := 16#1c#;
    constant ADDR_INPUT_DEPTH_V_DATA_0  : INTEGER := 16#20#;
    constant ADDR_INPUT_DEPTH_V_CTRL    : INTEGER := 16#24#;
    ...
    signal int_input_depth_V   : UNSIGNED(7 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_INPUT_DEPTH_V_DATA_0 =>
                        rdata_data <= RESIZE(int_input_depth_V(7 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    input_depth_V        <= STD_LOGIC_VECTOR(int_input_depth_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_INPUT_DEPTH_V_DATA_0) then
                    int_input_depth_V(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_input_depth_V(7 downto 0));
                end if;
            end if;
        end if;
    end process;
